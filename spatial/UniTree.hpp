...
public:
    ...
    void addData(T *thing) {
        // static uint depth = 0;
        // depth++;
        // std::cout << DEBUGVAR(depth) << std::endl;
        G dpos;
    ...
    std::shared_ptr<std::vector<T *>> getInArea(G &center, G &size, std::shared_ptr<std::vector<T *>> ret = std::make_shared<std::vector<T *>>()) {
        ...
        if (_dataMode){
            ...
            for (uint i = 0; i < std::pow(2, D); i++){
                _nexts[i]->getInArea(center, size, ret);
            }
        }
        return ret;
    }
