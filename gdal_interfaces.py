...
#file extracted from the open-elevation project
...
class GDALInterface(object):
    ...
    def lookup(self, lat, lon):
        try:
            ...
            xpixMin = math.floor(xpix)
            ylinMin = math.floor(ylin)

            xpixMax = xpixMin+1
            ylinMax = ylinMin+1
            
            xRatio = (self.geo_transform_inv[1] * u + self.geo_transform_inv[2] * v) - xpixMin
            yRatio = (self.geo_transform_inv[4] * u + self.geo_transform_inv[5] * v) - ylinMin

            # look the value up
            v00 = self.points_array[ylinMin, xpixMin]
            v10 = self.points_array[ylinMax, xpixMin]
            v01 = self.points_array[ylinMin, xpixMax]
            v11 = self.points_array[ylinMax, xpixMax]
            v = (v00*(1-yRatio) + v10*(yRatio)) * (1-xRatio) + (v01*(1-yRatio) + v11*(yRatio)) * (xRatio)
...
class GDALTileInterface(object):
    ...
    def lookup(self, lat, lng):
        ...
        if not nearest:
            ...
            return gdal_interface.lookup(lat, lng)
